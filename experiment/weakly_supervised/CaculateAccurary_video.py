...
def create_ellipse_f():
    ...
    if not os.path.exists(cfgs.list_save_path):
        with open(cfgs.list_save_path, 'w') as f:
            pass
...
class Ellip_acc(object):
    ...
    def divide_shelter_once(self, im, filename, pred, pred_pro, gt_ellip, if_valid=False, is_save=True):
        ...
        if is_save:
            ...
            new_line =  '%s %d %d %d %d %d\n' % (fn_full_path, ellipse_info[0][0], ellipse_info[0][1], ellipse_info[1][0], ellipse_info[1][1], 0)
