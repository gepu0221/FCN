...
import CaculateAccurary_find_thresh as accu
...
from train_Unet_parent_find_thresh import U_Net as FCNNet
...
class SeqFCNNet(FCNNet):
    ...
    def __init__(self, mode, max_epochs, batch_size, n_classes, train_records, valid_records, im_sz, init_lr, keep_prob, logs_dir):
        ...
        #Find probility thresh
        self.pro_find = tf.placeholder(tf.float32, shape=[None, cfgs.ANNO_IMAGE_SIZE[0], cfgs.ANNO_IMAGE_SIZE[1], 2], name='pro_find')
        self.test_thresh = tf.placeholder(tf.float32, name='test_thresh')
        self.find_thresh = cfgs.find_thresh
    ...
    def test_thresh_pro(self):
        
        sz = [self.cur_batch_size, cfgs.ANNO_IMAGE_SIZE[0], cfgs.ANNO_IMAGE_SIZE[1]]
        im_comp = tf.ones(sz, dtype=tf.int32)
        self.pred_anno_test_thresh = tf.expand_dims(tf.where(tf.less_equal(self.pro_find[:, :, :, 1], self.test_thresh), 1-im_comp, im_comp), dim=3)
            ...
            while count<self.per_e_valid_batch:
                ...
                for i in range(cfgs.test_thresh_num):
                    ...
                    test_pred_anno = sess.run(
                                              ...
                                              feed_dict={self.pro_find: pred_seq_pro,
                                                         self.test_thresh: thresh,
                                                         self.cur_batch_size: cfgs.batch_size})  
                    
                    test_pred_anno = np.squeeze(np.array(test_pred_anno), axis=0)
                    
                    self.calculate_acc(cur_ims[:, 2:cfgs.ANNO_IMAGE_SIZE[0]+2, :, :].copy(), filenames, test_pred_anno, pred_seq_pro, annos_, ellip_infos_, True, if_epoch)

                    if self.ellip_acc < min_ellip_loss:
                        min_ellip_loss = self.ellip_acc
                        min_thresh = thresh
                    thresh += cfgs.interval
                self.find_thresh = self.find_thresh * (1-cfgs.thresh_lr) + min_thresh * cfgs.thresh_lr
                ...
                line = 'epoch %5d\t lr = %g\t count = %4d\t loss = %.4f\t mean_loss=%.4f\t  valid_ellip_acc = %.2f\t choose_thresh = %.2f\t cur_thresh = %.3f\t time = %.2f' % (epoch, self.learning_rate, count, loss, (total_loss/count), min_ellip_loss, min_thresh, self.find_thresh, time_per_batch)
