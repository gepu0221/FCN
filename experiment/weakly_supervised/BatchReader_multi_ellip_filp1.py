...
def transform_rgb(filename):
    ...
    '''
        Add filp operator to cur_frame.
    '''
    ...
    #transform to gray
    resize_image = tf.image.rgb_to_grayscale(resize_image)

    #filp operator
    f_im1 = resize_image
    f_im2 = tf.image.flip_left_right(f_im1)
    f_im3 = tf.image.flip_up_down(f_im1)
    f_im4 = tf.image.flip_left_right(f_im3)

    f_im = tf.concat((f_im1, f_im2), 2)
    f_im = tf.concat((f_im, f_im3), 2)
    f_im = tf.concat((f_im, f_im4), 2)
...
def get_data_cache(files, batch_size, if_cache, variable_scope_name='get_data'):
    ...
    with tf.variable_scope(variable_scope_name) as var_scope:
        ...
        cur_im_batch.set_shape([batch_size, im_size[0], im_size[1], cfgs.cur_channel])
