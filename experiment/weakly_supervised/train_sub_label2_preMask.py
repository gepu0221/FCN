#This code is for training, adding pre frame mask and ||Mask(t-1)-Mask(t)||2 
...
class SeqFCNNet(FCNNet):
    ...
    def loss(self):
        ...
        self.loss1 = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits,
                                                                                       labels=tf.squeeze(self.annotations, squeeze_dims=[3]),
                                                                                       name='entropy_loss')))
        
        #Pre Mask
        self.pre_mask = self.images[:, :, :, cfgs.cur_channels]
        self.loss2 = tf.reduce_mean(tf.pow((self.pre_mask - self.logits[:, :, :, 2]), 2))
