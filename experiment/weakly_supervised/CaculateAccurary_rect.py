...
class Ellip_acc(object):
    ...
    def divide_shelter_once(self, im, filename, pred, pred_pro, gt_ellip, if_valid=False, is_save=True):
        ...
        if is_save:
            ...
            if ellipse_info[1][0] > ellipse_info[1][1]:
                rect_w = int(ellipse_info[1][0] / 2 * cfgs.expand_r) + 1
            else:
                rect_w = int(ellipse_info[1][1] / 2 * cfgs.expand_r) + 1
            cx = int(ellipse_info[0][1])
            cy = int(ellipse_info[0][0])
            lx = cx - rect_w
            rx = cx + rect_w + 1
            ly = cy - rect_w 
            ry = cy + rect_w + 1
            ...
            #im_ = np.append(im, im, axis=2)
            #im_ = np.append(im_, im, axis=2).astype(np.uint8)
            
            im_ = im[lx:rx, ly:ry, :]
            im = cv2.resize(im_, (cfgs.re_sz[0], cfgs.re_sz[1]), interpolation=cv2.INTER_CUBIC)
            
            #im = np.where(im>0, 1, 0)*255
            ...
            if loss > cfgs.loss_thresh:
                if loss > 500:
                    loss = 500
                #path_ = os.path.join(error_path, filename.strip().decode('utf-8')+'_'+str(int(loss))+'.bmp')
                path_ = os.path.join(error_path, filename.strip().decode('utf-8')+'.bmp')
