...
def read_txt_data(folder_dir, anno_folder):
    ...
    for txt_file in cfgs.file_list:
        ...
        while True:
            ...
            fn_simple = '%s' % (os.path.splitext(cur_frame_s[-1])[0])

            #Modify the path of current frame.
            m_cur_frame = os.path.join(cfgs.m_cur_path, fn_simple+'.bmp')
            ...
            anno_frame = os.path.join(anno_folder, fn_simple+'.bmp')

            #5. occlusion label
            label_occ_path = os.path.join(cfgs.class_label_path, fn_simple+'.bmp')
            label = 1
            if os.path.exists(label_occ_path):
                label = 0
            #6. generate record
            #if True:
            if os.path.exists(anno_frame):
                record = {'current':m_cur_frame, 'seq':seq_set, 'annotation':anno_frame, 'ellip_info':ellip_info, 'label':label, 'filename':filename}
                print(record)
...
def read_file_data():
    ...
    path_ = os.path.join(cfgs.file_dir)

    if not os.path.exists(path_):
        raise Exception('txt file folder %s not found' % path_)

    file_glob = os.path.join(path_, '*.bmp')
    file_list = []
