...
def save_gray_image(image, save_dir, name, mean=None):
...
def conv2d_strided_valid(x, W, b, stride=2):
...
def conv2d_transpose_strided_valid(x, W, b, output_shape=None, stride = 2):
    # print x.get_shape()
    # print W.get_shape()
    if output_shape is None:
        output_shape = x.get_shape().as_list()
...
def max_pool_diff(x, k_sz1, k_sz2, stride):
    return tf.nn.max_pool(x, ksize=[1, k_sz1, k_sz2, 1], strides=[1, stride, stride, 1], padding="SAME")


def max_pool_valid(x, kernel_size, stride):
    return tf.nn.max_pool(x, ksize=[1, kernel_size, kernel_size, 1], strides=[1, stride, stride, 1], padding="VALID")
...
def avg_pool_diff(x, k_sz1, k_sz2, stride):
    return tf.nn.avg_pool(x, ksize=[1, k_sz1, k_sz2, 1], strides=[1, stride, stride, 1], padding="VALID")
...
def clear_line(line_width):
    
    '''
    Clear line using line_width blank space.
    '''
    print('\r' + line_width * ' ' + '\r', end='')
