    ...
    def valid_once(self, sess, writer, epoch, step):
        ...
        try:
            ...
            while count<self.per_e_valid_batch:
                ...
                line = 'epoch %5d\t lr = %g\t count = %4d\t loss = %.4f\t mean_loss=%.4f\t  train_ellip_acc = %.2f\t choose_thresh = %.2f\t cur_thresh = %.3f\t time = %.2f' % (epoch, self.learning_rate, count, loss, (total_loss/count), min_ellip_loss, min_thresh, self.find_thresh, time_per_batch)
