#This code is for genreating the segmetation part of corean of I(t-1), which part is occluded by instrument on I(t).
...
class SeqFCNNet(FCNNet):
    ...
    def generate_mask(self):
        ...
        self.inst_mask_pro = tf.placeholder(tf.float32, shape=[None, self.IMAGE_SIZE[0], self.IMAGE_SIZE[1], cfgs.cur_channel], name='mask_pro')
        self.corn_mask_pro = tf.placeholder(tf.float32, shape=[None, self.IMAGE_SIZE[0], self.IMAGE_SIZE[1], cfgs.cur_channel], name='mask_pro')
        ...
        self.persist_im = tf.multiply(self.inst_mask, self.corn_mask)
         
    def part_filter(self):
        '''
            Use mask to filter im.
        '''
        self.filter_mask = tf.placeholder(tf.float32, shape=[None, self.IMAGE_SIZE[0], self.IMAGE_SIZE[1], 1], name='mask')
        self.filter_im = tf.placeholder(tf.float32, shape=[None, self.IMAGE_SIZE[0], self.IMAGE_SIZE[1], 1], name='im_filter')
        self.persist_im = tf.multiply(self.filter_mask, self.filter_im)
    ...
    def view_one(self, fn, pred_anno, pred_pro, im, step):
        ...
        #pdb.set_trace()
        pred_anno *= 127
        prev_warped = pred_anno
        #prev_warping = cv2.cvtColor(pred_anno.astype(np.uint8), cv2.COLOR_BGR2RGB)
        cv2.imwrite(os.path.join(path_, filename+'_warp.bmp'), prev_warped)
    ...
    def train_one_epoch(self, sess, data_loader, data_num, epoch, step):
        ...
        for count in range(1, data_num):
            ...
            #pdb.set_trace()
            self.view(np.expand_dims(fn, 0), anno_prev_warped, images[cfgs.seq_frames-2], images[cfgs.seq_frames-1], step)
            ...
            cv2.imwrite(os.path.join('persist_inter1', '%s_persist.bmp' % fn), persist_im[0]*255)
            #pdb.set_trace()
