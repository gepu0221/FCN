...
class DataLoader_c():
    ...
    def get_next_sequence(self):
        ...
        #get param for spatial discounting mask
        fn_split = fn.split('_')
        # [i0, j0, h, w] for tensorflow
        rect_param = [int(int(fn_split[2])*rate2),int(int(fn_split[3])*rate1),\
                      int(int(fn_split[4])*rate2), int(int(fn_split[5])*rate1)]
        ...
        dt = -1
        while True:
            inter = (np.random.randint(10)+1) % (cfgs.inter+1)
            t = int(frame) + dt * inter
        ...
        return images, images_da, mask, fn, flag, rect_param

    def get_next_sequence_valid(self):
        ...
        dt = -1
        while True:
            t = int(frame) + dt * int(cfgs.inter/2)
