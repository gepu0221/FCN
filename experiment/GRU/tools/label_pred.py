import numpy as np
from tools.ellipse_my import ellipse_my
import cv2
import time
import pdb
soft_thred = 0.95
...
def pred_visualize_choose_color(image, pred, color=[0,255,0]):
    i_sz=image.shape
    for i in range(i_sz[0]):
        for j in range(i_sz[1]):
            if pred[i][j]!=0:
...
#label the prediction pixels at origin size image.
def pred_ori_visualize(image,pred):
    i_sz=pred.shape
    im_sz = image.shape
    rate0 = im_sz[0] / i_sz[1]
    rate1 = im_sz[1] / i_sz[0]
    rate = [rate0, rate1]
