import cv2
from tools.EFCHandler import EFCHandler
import numpy as np
...
def EllipseEx_my(center,axes,angle,arc_start,arc_end):
    ...
    efc=EFCHandler()
    part_label=efc.Part(pts,coef_num,part_num)
    
    return part_label

def EllipseEx_my_pt(center,axes,angle,arc_start,arc_end):
  
    '''
    return ellipse contour points set.
    '''
    #Size:(axes.width,axes.height)
    axes[0]=abs(axes[0])
    axes[1]=abs(axes[1])
...
#[x,y,width,height]
#e_info((center.x,center.y),(axis.w,axis.h),angle)
def ellipse_my(e_info):
    assert e_info[1][0]>=0 and e_info[1][1]>=0
    _angle=round(e_info[2])
   
    #enter=np.array(e_info[0])
    center=np.array([e_info[0][1],e_info[0][0]])
    axes=np.array([e_info[1][0]/2,e_info[1][1]/2])
    v=EllipseEx_my(center,axes,_angle,0,360)
    
    return v
 

def ellipse_my_pt(e_info):

    assert e_info[2]>=0 and e_info[3]>=0
    _angle=0
   
    #enter=np.array(e_info[0])
    center=np.array([e_info[1],e_info[0]])
    axes=np.array([e_info[2]/2,e_info[3]/2])
    v=EllipseEx_my_pt(center,axes,_angle,0,360)
