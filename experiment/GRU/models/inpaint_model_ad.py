...
class InpaintCAModel(Model):
    ...
    def build_graph_with_losses(self, batch_data, config, training=True,
        ...
        losses['l1_loss'] = l1_alpha * (tf.reduce_mean(tf.abs(batch_pos - x1) * mask) / num_mask)
        if not config.PRETRAIN_COARSE_NETWORK:
            losses['l1_loss'] = tf.reduce_mean(tf.abs(batch_pos - x2) * mask) / num_mask

        losses['l2_loss'] /= tf.reduce_mean(mask)

        #losses['ae_loss'] = l1_alpha * tf.reduce_mean(tf.abs(batch_pos - x1) * (1.-mask))
        #r_num_mask = tf.reduce_sum(1-mask)
        losses['ae_loss'] = l1_alpha * tf.reduce_mean(tf.abs(batch_pos - x1) * (1.-mask))
