...
class LRR:
    ...
    def get_blobs(self, im, dropout_keeprate=1):
        ...
        blobs['prediction_32x'] = tf.concat([tf.nn.conv2d_transpose(tf.slice(blobs['coef_32x'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), tf.slice(self.weights['deconv_32xf'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), output_shape=[batch_size, ADAPTED_HEIGHT//8, ADAPTED_WIDTH//8, 1], strides=[1,4,4,1]) for k in range(num_class)], axis=3)
        ...
        blobs['dil_seg32x'] = tf.concat([tf.nn.conv2d_transpose(tf.slice(blobs['dil_seg32x_coef'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), tf.slice(self.weights['dil_seg_deconv_32xf'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), output_shape=[batch_size, ADAPTED_HEIGHT//8, ADAPTED_WIDTH//8, 1], strides=[1,4,4,1]) for k in range(num_class)], axis=3) + self.weights['dil_mask_deconv32sb']
        ...
        blobs['ero_seg32x'] = tf.concat([tf.nn.conv2d_transpose(tf.slice(blobs['ero_seg32x_coef'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), tf.slice(self.weights['ero_seg_deconv_32xf'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), output_shape=[batch_size, ADAPTED_HEIGHT//8, ADAPTED_WIDTH//8, 1], strides=[1,4,4,1]) for k in range(num_class)], axis=3) + self.weights['ero_mask_deconv32sb']

        blobs['coef_16x'] = tf.nn.conv2d(blobs['x30'], self.weights['bases_coef_16sf'], [1,1,1,1], padding='SAME') + self.weights['bases_coef_16xb']

        blobs['prediction_16x_add'] = tf.concat([tf.nn.conv2d_transpose(tf.slice(blobs['coef_16x'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), tf.slice(self.weights['deconv_16xf'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), output_shape=[batch_size, ADAPTED_HEIGHT//4, ADAPTED_WIDTH//4, 1], strides=[1,4,4,1]) for k in range(num_class)], axis=3)

        
        blobs['prediction_32x_bil_x2'] = tf.concat([tf.nn.conv2d_transpose(tf.slice(blobs['prediction_32x'], [0, 0, 0, k], [-1, -1, -1, 1]), tf.slice(self.weights['dec_prediction_32x_bil_x2f'], [0, 0, 0, k], [-1, -1, -1, 1]), output_shape=[batch_size, ADAPTED_HEIGHT//4, ADAPTED_WIDTH//4, 1], strides=[1,2,2,1]) for k in range(num_class)], axis=3)
        ...
        blobs['prediction_8x_add'] = tf.concat([tf.nn.conv2d_transpose(tf.slice(blobs['coef_8x'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), tf.slice(self.weights['deconv_8xf'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), output_shape=[batch_size, ADAPTED_HEIGHT//2, ADAPTED_WIDTH//2, 1], strides=[1,4,4,1]) for k in range(num_class)], axis=3)

        blobs['prediction_16x'] = blobs['prediction_32x_bil_x2'] + blobs['pred_16x_aft_DP']

        blobs['prediction_16x_bil_x2'] = tf.concat([tf.nn.conv2d_transpose(tf.slice(blobs['prediction_16x'], [0, 0, 0, k], [-1, -1, -1, 1]), tf.slice(self.weights['dec_prediction_16x_bil_x2f'], [0, 0, 0, k], [-1, -1, -1, 1]), output_shape=[batch_size, ADAPTED_HEIGHT//2, ADAPTED_WIDTH//2, 1], strides=[1,2,2,1]) for k in range(num_class)], axis=3)
        ...
        blobs['prediction_4x_add'] = tf.concat([tf.nn.conv2d_transpose(tf.slice(blobs['coef_4x'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), tf.slice(self.weights['deconv_4xf'], [0, 0, 0, 10*k], [-1, -1, -1, 10]), output_shape=[batch_size, ADAPTED_HEIGHT, ADAPTED_WIDTH, 1], strides=[1,4,4,1]) for k in range(num_class)], axis=3)


        blobs['prediction_8x_bil_x2'] = tf.concat([tf.nn.conv2d_transpose(tf.slice(blobs['prediction_8x'], [0, 0, 0, k], [-1, -1, -1, 1]), tf.slice(self.weights['dec_prediction_8x_bil_x2f'], [0, 0, 0, k], [-1, -1, -1, 1]), output_shape=[batch_size, ADAPTED_HEIGHT, ADAPTED_WIDTH, 1], strides=[1,2,2,1]) for k in range(num_class)], axis=3)
    ...
    def all_variables(self):
        return [('deconv_16xf', (8, 8, 1, num_class*10)),
            ('conv3_1b', (256,)), 
            ('bases_coef_32xb', (num_class*10,)),
            ('dil_seg32x_coeff', (5, 5, 4096, num_class*10)),
            ...
            ('bases_coef_16xb', (num_class*10,)),
            ('dil_seg32x_coefb', (num_class*10,)),
            ('deconv_32xf', (8, 8, 1, num_class*10)),
            ('bases_coef_16sf', (5, 5, 512, num_class*10)),
            ('ero_mask32s_coeff', (5, 5, 4096, num_class*10)),
            ('conv4_1b', (512,)), 
            ('bases_coef_32xf', (5, 5, 4096, num_class*10)),
            ('conv2_2b', (128,)), 
            ('conv1_2b', (64,)),  
            ('conv3_3f', (3, 3, 256, 256)),
            ('dec_prediction_8x_bil_x2f', (4, 4, 1, num_class)),
            ('conv3_2b', (256,)), 
            ('bases_coef_8xb', (num_class*10,)),
            ('fc7f', (1, 1, 4096, 4096)),
            ('ero_seg_deconv_32xf', (8, 8, 1, num_class*10)),
            ('conv2_1b', (128,)), 
            ('conv1_2f', (3, 3, 64, 64)),
            ('fc6b', (4096,)),
            ('conv4_2b', (512,)), 
            ('ero_seg32x_coefb', (num_class*10,)),
            ('bases_coef_8xf', (5, 5, 512, num_class*10)),
            ('conv5_2f', (3, 3, 512, 512)),
            ('conv1_1b', (64,)),  
            ('conv5_1f', (3, 3, 512, 512)),
            ('bases_coef_4xb', (num_class*10,)),
            ('conv3_1f', (3, 3, 128, 256)),
            ('dec_prediction_16x_bil_x2f', (4, 4, 1, num_class)),
            ('conv3_3b', (256,)), 
            ('conv2_1f', (3, 3, 64, 128)),
            ('deconv_4xf', (8, 8, 1, num_class*10)),
            ('dil_mask_deconv32sb', (num_class,)),
            ('conv5_3b', (512,)), 
            ('fc7b', (4096,)),
            ('conv2_2f', (3, 3, 128, 128)),
            ('conv4_3b', (512,)), 
            ('conv5_1b', (512,)), 
            ('bases_coef_4xf', (5, 5, 256, num_class*10)),
            ('conv4_1f', (3, 3, 256, 512)),
            ('dec_prediction_32x_bil_x2f', (4, 4, 1, num_class)),
            ('deconv_8xf', (8, 8, 1, num_class*10)),
            ('conv1_1f', (3, 3, 3, 64)),
            ('dil_seg_deconv_32xf', (8, 8, 1, num_class*10)),
            ('ero_mask_deconv32sb', (num_class,))]
