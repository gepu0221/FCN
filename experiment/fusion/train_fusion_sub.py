...
class SeqFCNNet(FCNNet):
    ...
    def fusion(self):
        ...
        self.logits = tf.add(self.sm_logits, self.soft_logits)
    ...
    def loss(self):
        ...
        self.true_loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits,
                                                                                        labels=tf.squeeze(self.sm_annos, squeeze_dims=[3]),
                                                                                        name='entropy_mask')))

        self.loss = tf.add(self.true_loss, self.soft_loss)
        #test 
    ...
    def sm_loss(self):
        self.sm_pro = tf.nn.softmax(self.sm_logits)

    def soft_loss(self):
        soft_logits = self.soft_logits / cfgs.temperature
        self.soft_pro = tf.nn.softmax(self.soft_logits)
        self.soft_loss = tf.reduce_mean((tf.nn.softmax_cross_entropy_with_logits(logits=soft_logits,
                                                                            labels=self.soft_annos,
                                                                            name="entropy")))
    ...
    #6. else
    def create_view_f(self):
        root_path = os.path.join('image', cfgs.view_path)
        if not os.path.exists(root_path):
            os.makedirs(root_path)
            print(root_path)
        train_path = os.path.join('image', cfgs.view_path, 'train')
        valid_path = os.path.join('image', cfgs.view_path, 'valid')
    ...
    def view_one(self, fn, pred_anno, pred_pro, sm_pred_pro, soft_pred_pro, im, step):
        path_ = os.path.join('image', cfgs.view_path, 'train')
        ...
        cv2.imwrite(os.path.join(path_, filename+'_soft_heat.bmp'), heatmap_soft)
    ...
    def view(self, fns, pred_annos, pred_pros, sm_pred_pros, soft_pred_pros, ims, step):
        if cfgs.random_view:
            choosen = random.randint(0, self.batch_size-1)
            self.view_one(fns[choosen], pred_annos[choosen], pred_pros[choosen], sm_pred_pros[choosen], soft_pred_pros[choosen], ims[choosen], step)
        else:
            for i in range(self.batch_size):
                self.view_one(fns[i], pred_annos[i], pred_pros[i], sm_pred_pros[i], soft_pred_pros[i], ims[i], step)   

    def view_one_valid(self, fn, pred_anno, pred_pro, sm_pred_pro, soft_pred_pro, im, step):
    ...
    def view_valid(self, fns, pred_annos, pred_pros, sm_pred_pros, soft_pred_pros, ims, step):
        if cfgs.random_view:
            choosen = random.randint(0, self.batch_size-1)
            self.view_one_valid(fns[choosen], pred_annos[choosen], pred_pros[choosen], sm_pred_pros[choosen], soft_pred_pros[choosen], ims[choosen], step)
        else:
            for i in range(self.batch_size):
                self.view_one_valid(fns[i], pred_annos[i], pred_pros[i], sm_pred_pros[i], soft_pred_pros[i], ims[i], step)   
    ...
    def valid_once(self, sess, writer, epoch, step):
        ...
        try:
            ...
            while count<self.per_e_valid_batch:
                count +=1
                images_, cur_ims, ellip_infos_, annos_, soft_annos_, filenames = sess.run([self.valid_images, self.valid_cur_ims, self.valid_ellip_infos, self.valid_annotations, self.valid_annos_soft, self.valid_filenames])
                pred_anno, pred_seq_pro, pred_sm_pro, pred_soft_pro, summary_str, loss= sess.run(
                #1. valid
                #fetches=[self.pred_annotation, self.pro, self.sm_pro, self.soft_pro, self.summary_op, self.loss],
                #2. valid lower
                fetches=[self.pred_anno_lower, self.pro, self.sm_pro, self.soft_pro, self.summary_op, self.loss],
                feed_dict={self.sm_images: images_, 
                          self.soft_images: cur_ims, 
                           self.sm_annos: annos_, self.lr: self.learning_rate,
                           self.soft_annos: soft_annos_,
                           self.input_keep_prob: 1.0,
                           self.keep_prob: 1.0
                           })

                #self.view_valid(filenames, pred_anno, pred_seq_pro, pred_sm_pro, pred_soft_pro, images_, step)
    ...
    def train_one_epoch(self, sess, writer, epoch, step):
        ...
        try:
            ...
            while count<self.per_e_train_batch:
                ...
                count += 1
                #1. train
                images_, cur_ims_, ellip_infos_, annos_, soft_annos_, filenames = sess.run([self.train_images, self.train_cur_ims, self.train_ellip_infos, self.train_annotations, self.train_annos_soft, self.train_filenames])
                
                #1. train
                #pred_anno_, pred_seq_pro_, pred_sm_pro, pred_soft_pro, summary_str, loss, _ = sess.run([self.pred_annotation, self.pro, self.sm_pro, self.soft_pro, self.summary_op, self.loss, self.train_op],
                #2. not train- lower
                pred_anno_, pred_seq_pro_, pred_sm_pro, pred_soft_pro, summary_str, loss = sess.run([self.pred_anno_lower, self.pro, self.sm_pro, self.soft_pro, self.summary_op, self.loss],
                #3. not train
                #pred_anno_, pred_seq_pro_, pred_sm_pro, pred_soft_pro, summary_str, loss = sess.run([self.pred_annotation, self.pro, self.sm_pro, self.soft_pro, self.summary_op, self.loss],

                                                                feed_dict={self.soft_images: cur_ims_, 
                                                                           self.sm_images: images_,
                                                                           self.sm_annos: annos_,
                                                                           self.soft_annos: soft_annos_,
                                                                           self.input_keep_prob: cfgs.input_keep_prob,
                                                                           self.keep_prob: cfgs.keep_prob,
                                                                           self.lr: self.learning_rate})
                
                #2. calculate accurary
                self.view(filenames, pred_anno_, pred_seq_pro_, pred_sm_pro, pred_soft_pro, images_, step)
