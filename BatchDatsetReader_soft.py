...
class BatchDatset:
    ...
    def _read_images(self):
        ...
        anno_ = np.array([self._transform(filename['annotation']) for filename in self.files])
        self.annotations = np.delete(anno_, -1, axis=3)
        #print('shape of anno', anno_.shape)
        #print('shape of part anno', np.expand_dims(anno_[:,:,:,1], axis=3).shape)
        #self.annotations = np.concatenate((np.expand_dims(anno_[:,:,:,2], axis=3), np.expand_dims(anno_[:,:,:,1], axis=3)), axis=3)
