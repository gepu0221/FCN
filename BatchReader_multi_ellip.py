...
def _read_images_list(files, shuffle=True):
    ...
    data = tf.data.Dataset.from_tensor_slices((cur_images, ellip_infos, annotations, filenames))
...
#----------------Parse List Begin-------------
def _parse_record(cur_filename, ellip_info, anno_filename, filename):
    ...
    return im, ellip_info, anno_im, filename


def _parse_vis_record(cur_filename, ellip_info, anno_filename, filename):
...
def get_data_cache(files, batch_size, if_cache, variable_scope_name='get_data'):
    ...
    with tf.variable_scope(variable_scope_name) as var_scope:
        ...
        #5. fetch and return
        im_batch, ellip_info_batch, anno_im_batch, filename_batch = data_iter.get_next()
    ...
    return im_batch, ellip_info_batch, anno_im_batch, filename_batch
...
def get_data_vis(vis_files, batch_size, variable_scope_name='get_data'):
    ...
    with tf.variable_scope(variable_scope_name) as var_scope:
        ...
        #5. fetch and return
        im_batch, ellip_info_batch, anno_im_batch, filename_batch = iterator.get_next()
    ...
    return im_batch, ellip_info_batch, anno_im_batch, filename_batch, vis_init
